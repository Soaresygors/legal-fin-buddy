  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from('lancamentos')
        .select('*, plano_contas(codigo, descricao, grupo), centros_custo(nome)')
        .eq('tipo', 'D')
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{formatCurrency(totalPago)}</div>
            <p className="text-xs text-muted-foreground">{pagos.length} pagos</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Despesas {selectedYear}</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left py-3 px-2 font-medium text-muted-foreground hidden lg:table-cell">Centro Custo</th>
                  ...
                  {contas.map((c) => (
                    <tr key={c.id} className="border-b border-border/50 hover:bg-muted/30">
                      ...
                      <td className="py-2.5 px-2 text-xs">
                        <span className="font-mono text-muted-foreground">{c.plano_contas?.codigo}</span>{' '}
                        {c.plano_contas?.descricao}
                      </td>
                      <td className="py-2.5 px-2 hidden md:table-cell truncate max-w-[250px]">{c.descricao}</td>
                      <td className="py-2.5 px-2 hidden lg:table-cell">{c.centros_custo?.nome || '-'}</td>
                      <td className="py-2.5 px-2 text-right font-medium text-destructive">{formatCurrency(c.valor_realizado)}</td>
